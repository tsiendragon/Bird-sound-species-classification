    ...
    for i in range(bsz):
        ...
        if no_overlap:
            ...
            def arrange(s, e, length, keep_length):
                ...
                mask_idc.extend(span_start + i for i in range(length))  # noqa
...
class WavLMConfig:
    def __init__(self, cfg=None):
        ...
        # mode for feature extractor. default has a single group norm with d groups in the
        # first conv block, whereas layer_norm has layer norms in every block (meant to use with normalize=True)
        ...
        # string describing convolutional feature extraction layers in form of a python list that
        ...
        self.encoder_layerdrop: float = (
            0.0  # probability of dropping a transformer layer
        )
        self.dropout_input: float = (
            0.0  # dropout to apply to the input (after feat extractor)
        )
        self.dropout_features: float = (
            0.0  # dropout to apply to the features (after feat extractor)
        )
