...
class GLU_Linear(nn.Module):
    ...
    def forward(self, x):
        # to be consistent with GLU_Linear, we assume the input always has the #channel (#dim) in the
        # last dimension of the tensor, so need to switch the dimension first for 1D-Conv case
        x = self.linear(x)
...
class MultiheadAttention(nn.Module):
    ...
    def forward(  # NOQA
        ...
        attn_weights: Optional[Tensor] = None  # type: ignore
        if need_weights:
